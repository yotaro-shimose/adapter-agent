...
class KnowledgeSlicer[T: AgentsSDKModel](BaseAgent[T]):
    async def slice(self, normalized_knowledge: str) -> QRA | None:
        """
...
Your goal is to generate a high-quality practice problem (QRA) based on the provided "Normalized Knowledge".

The QRA must consist of:
...
OUTPUT FORMAT:
Provide your reasoning, and then output exactly one QRA inside <qra></qra> xml block.
Inside <qra>, use <question>, <reasoning>, and <answer> tags.
...
""",
        ...
        qra_match = re.search(r"<qra>(.*?)</qra>", response_text, re.DOTALL)
        if not qra_match:
            logger.warning("Could not find <qra> block in KnowledgeSlicer response.")
            return None

        block = qra_match.group(1)
